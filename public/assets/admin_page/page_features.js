export default {
        ...
        <div class="bd-intro pt-2 ps-lg-2">
            <div class="d-md-flex flex-md-row-reverse align-items-center justify-content-between">
                <div class="mb-3 mb-md-0 d-flex">
                    <router-link :to="{path: '/app/article', query: {type: 'article'}}" class="btn btn-sm btn-bd-light rounded-2">New Article</router-link>
                </div>
                <h1 class="bd-title mb-0" id="content">Daftar Feature Article</h1>
            </div>
            <p class="bd-lead">Ini adalah daftar feature article yang telah dibuat</p>
        </div>
            ...
            <div class="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative" v-for="item in articles">
                <div class="col p-4 d-flex flex-column position-static">
                    <strong class="d-inline-block mb-2 text-primary">
                        <router-link :to="{path: '/app/category', query: { id : cat.id }}" class="text-decoration-none" v-for="cat in item.category">
							<span class="badge rounded-pill text-bg-primary">{{ cat.category_name }}</span> 
						</router-link>
                    </strong>
                    <h3 class="mb-0"><a class="text-decoration-none" :href="item.link" v-text="item.article_title"></a></h3>
    ...
    <div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasRight" ref="offcanvasRight" aria-labelledby="offcanvasRightLabel">
        ...
        <div class="offcanvas-body d-flex flex-nowrap">
            <form @submit.prevent="store" class="w-100 d-flex flex-column flex-shrink-0 p-1 text-bg-dark">
                ...
                <div class="card">
                    <img class="card-img" width="100%" height="140" :src="article_feature" style="object-fit: cover;object-position: center center;">
                </div>
                <div class="input-group mt-1 mb-3">
    ...
    methods: {
        refresh() {
            fetch(`/api/article?page=${this.page}`, {
                ...
                .then((e) => e.json())
                .then((res) => {
                    this.articles = res.data.features;
        ...
        showPublish(id) {
            ...
            if (articleFilter.length) {
                ...
                this.article_cat = article.category ? article.category : [];
            }
        },
        categoryToJSON(cat) {
    ...
    mounted() {
        ...
        this.$refs.offcanvasRight.addEventListener(
            "show.bs.offcanvas",
            (event) => {
